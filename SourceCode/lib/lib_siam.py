...
def train_SiameseNetwork(model_name,siamese_model, train_loader, test_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        final_accuracy = val_accuracy_pos + val_accuracy_neg
        if final_accuracy > best_val_accuracy:
            best_val_accuracy = final_accuracy
            patience_counter = 0
            torch.save(siamese_model.state_dict(), f'SaveModel/classifier_{model_name}.pth')
        else:
            patience_counter += 1
...
def save_loaders(model_name, test_loader, mean_feature):
    output_dir =f"NeckpassDataset/{model_name}"
