...
if __name__ == "__main__":
    ...
    tmp_model_path1 = get_model_path()
    tmp_model_path2 = get_model_path("Genetics")
    test_scenario = os.listdir(Input_path)
    scenario_result = []
    print(f"No, User_name      ----   TPR,      TNR,      BAC")
    result_list = []
    for jj, scenario in enumerate(test_scenario):
        ...
        for i in range(loop):
            ...
            if model_name in ["User33", "User34", "User35"]:
                model_path = f'{tmp_model_path2}/Model_Am/Am_{model_name}.pth'
                mean_feature = torch.load(f'{tmp_model_path2}/Dataset_NeckPass/{model_name}/mean_feature.pt')
            else:
                model_path = f'{tmp_model_path1}/Model_Am/Am_{model_name}.pth'
                mean_feature = torch.load(f'{tmp_model_path1}/Dataset_NeckPass/{model_name}/mean_feature.pt')
            ...
            TPR = TPR_dic[model_name][0]
            TPR_list.append(TPR)
            TNR = (1 - count_pos/len(pos_list))*100 if len(pos_list) > 0 else 0
            TNR_list.append(TNR)
            BAC = (TPR + TNR) / 2
            BAC_list.append(BAC)
            User_name.append(model_name)
        avgTPR = np.mean(TPR_list)
        avgTNR = np.mean(TNR_list)
        avgBAC= np.mean(BAC_list)
        result_list.append([avgTPR,avgTNR,avgBAC])
        print(f"{jj:<2},{scenario:<12}    ----   {avgTPR:.2f}%,   {avgTNR:.2f}%,   {avgBAC:.2f}%")
    result_array = np.array(result_list)
    avg_values = np.mean(result_array, axis=0)
    print(f"\n[Average]    ----   {avg_values[0]:.2f}%,   {avg_values[1]:.2f}%,   {avg_values[2]:.2f}%")