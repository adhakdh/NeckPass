...
if __name__ == "__main__":
    ...
    for vote in range(1,6,1):
        ...
        for i in range(loop):
            ...
            model_path = f'{tmp_model_path}/Model_Am/Am_{model_name}.pth'
            input_dim = all_users_data[0][0].shape[0]
            model = load_model(model_path, input_dim)
            mean_feature = torch.load(f'{tmp_model_path}/Dataset_NeckPass/{model_name}/mean_feature.pt')
            ...
            for ii, (data, file_name, label) in enumerate(all_users_data):
                ...
                if label == "1":
                    ...
                    if preds_label:
                        ...
                        neg_list.append(0)
            pos_list = get_paired(pos_list,vote)
            neg_list = get_paired(neg_list,vote)
        ...
        vote_result.append([TPR_list, TNR_list, BAC_list])

    title_str = f"No, User_name      ----   "
    title_str += ',   '.join(f"TPR{vote+1}" for vote in range(len(vote_result)))
    title_str += f"    ----   "
    title_str += ',   '.join(f"TNR{vote+1}" for vote in range(len(vote_result)))
    print(title_str)

    for i, (user_name) in enumerate(User_name):
        result_str = f"{i:<2},{user_name:<15} ----   "
        result_str += ',   '.join(f"{vote_result[vote][0][i]:.2f}%" for vote in range(len(vote_result)))
        result_str += f"    ----   "
        result_str += ',   '.join(f"{vote_result[vote][1][i]:.2f}%" for vote in range(len(vote_result)))
        print(result_str)

    avg_str = f"\n[Average]    ----   "
    avg_str += ',   '.join(f"{np.mean(vote_result[vote][0]):.2f}%" for vote in range(len(vote_result)))
    avg_str += f"    ----   "
    avg_str += ',   '.join(f"{np.mean(vote_result[vote][1]):.2f}%" for vote in range(len(vote_result)))
    print(avg_str)