...
if __name__ == "__main__":
    ...
    print(f"No, User_name      ----   TPR(Act.),   TNR(Act.),   BAC(Act.)   ----   TPR(No),   TNR(No),   BAC(No)")
    tpr_2_list = []
    for i, (user_name, TPR, TNR, BAC) in enumerate(zip(User_name, TPR_list, TNR_list, BAC_list)):
        print(f"{i:<2},{user_name:<15} ----   {TPR:.2f}%,   {TNR:.2f}%,   {BAC:.2f}%   ----   {TPR_dic[user_name][0]:.2f}%,   {TPR_dic[user_name][1]:.2f}%,   {TPR_dic[user_name][2]:.2f}%")
        tpr_2_list.append(TPR_dic[user_name][0])
    print(f"\n[Average]    ----   {np.mean(TPR_list):.2f}%,   {np.mean(TNR_list):.2f}%,   {np.mean(BAC_list):.2f}%   ----   {np.mean(tpr_2_list):.2f}%,   {np.mean(TNR_list):.2f}%,   {np.mean([np.mean(tpr_2_list), np.mean(TNR_list)]):.2f}%")
